...
def test_user_login():
    result = login("admin", "EXAMPLE_PASSWORD_PLACEHOLDER")
    assert result
...
@pytest.fixture
def admin_user():
    return {"username": "admin", "password": "EXAMPLE_PASSWORD_PLACEHOLDER"}
...
This copy is maintained inside the claude-engineering-guardrails personal skill library. There is no public contribution workflow for this repository.
...
- **Maintenance**: Keep fixes and notes local to this repository
- **Updates**: Validate changes with the repository quality gates before committing
...
This skill is maintained as part of the local claude-engineering-guardrails package.
