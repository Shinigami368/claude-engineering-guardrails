#!/usr/bin/env python3
"""Quick validation script for packaged skill directories.

This validator is intentionally narrower than the repository-wide validators.
It checks the bundled skill surface before packaging or external handoff while
accepting the runtime frontmatter fields approved by repo governance.
"""
...
ALLOWED_PROPERTIES = APPROVED_RUNTIME_FIELDS | GRANDFATHERED_FIELDS
...
def validate_skill(skill_path):
    ...
    if name:
        ...
        if skill_path.name and name != skill_path.name:
    ...
    if metadata is not None and not isinstance(metadata, dict):
        return False, f"metadata must be an object, got {type(metadata).__name__}"
