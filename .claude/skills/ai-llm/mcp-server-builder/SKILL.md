...
description: "Builds Model Context Protocol servers implementing tool and resource handlers"
---
...
- Decide when a capability belongs as a tool, resource, or prompt
...
- Use stdio for local single-user clients; use Streamable HTTP for remote or multi-client servers.
- Apply `references/runtime-contract-gates.md` before installing into a real MCP client.
...
- Keep short contract migration notes only when versioned server releases exist.
...
7. Installing into a client before `inspect`, `list`, and real tool calls pass
...
6. Pair breaking contract changes with migration notes when releases exist.
...
- [references/runtime-contract-gates.md](references/runtime-contract-gates.md)
- [README.md](README.md)
...
- Keep backward compatibility for one compatibility window minimum.
- Keep contract migration notes for new/removed/changed tools only when needed.
